  ...
  return (
    <div id={slug} className="scroll-mt-24 h-full flex flex-col">
      <Card className="h-full flex flex-col">
        <p className="text-accent text-xs sm:text-[10px] font-bold tracking-wide sm:tracking-widest uppercase mb-3 leading-snug">
          {label}
        </p>
...
export default function TradesYarn() {
  ...
  return (
    <>
      <section className="relative pt-6 pb-8 sm:pt-8 sm:pb-10 md:pt-10 md:pb-12 overflow-hidden">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 grid lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-10 items-center">
          <ScrollReveal className="min-w-0">
            ...
            <h1 className="font-display text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-3 leading-tight">
              Every Count. Every Composition. Sourced Right.
            </h1>
            <p className="text-text-secondary text-sm sm:text-base md:text-lg mb-5 sm:mb-6 leading-relaxed">
              From Ne 6 cotton to compact polyester blends — we hold stock and source to spec for mills worldwide.
            </p>
            <Button to="/trades/rfq" size="lg" className="w-full sm:w-auto">Request a Quote →</Button>
          </ScrollReveal>
          <div className="h-48 sm:h-56 md:h-64 lg:h-72 rounded-2xl border border-border overflow-hidden bg-bg-elevated min-w-0">
      ...
      <section className="pb-10 sm:pb-12 md:pb-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 space-y-10 sm:space-y-12">
      ...
      <section className="pb-12 md:pb-16 bg-bg-secondary">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 pt-8 sm:pt-10">
          <h2 className="font-display text-lg sm:text-xl font-semibold mb-5 sm:mb-6 text-center">Sourcing Process</h2>
              ...
              (s, i) => (
                <ScrollReveal key={s} delay={i * 0.05}>
                  <div className="text-center p-2 sm:p-3">
                    <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-accent/20 text-accent text-xs sm:text-sm font-bold flex items-center justify-center mx-auto mb-1.5 sm:mb-2">
                      {i + 1}
                    </div>
                    <p className="text-[10px] sm:text-xs text-text-secondary leading-snug">{s}</p>
                  </div>
                </ScrollReveal>
              ),
            )}
